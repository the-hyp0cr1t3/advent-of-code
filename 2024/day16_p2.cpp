...
int main() {
    ...
    auto dijkstra = [&](int rx, int ry, std::vector<int> start_dirs) {
        std::vector<i64> dist(n * m * 4, inf);
        std::priority_queue<state> pq;
        for (auto d : start_dirs)
            pq.emplace(rx, ry, d, dist[f(rx, ry, d)] = 0);
    ...
    auto sd = dijkstra(sx, sy, {1});
    auto ed = dijkstra(ex, ey, {0, 1, 2, 3});

    int ans = 0;
    i64 best = ed[f(sx, sy, 3)];
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            bool good = false;
            for (int d = 0; d < 4; d++)
                good |= (sd[f(i, j, d)] + ed[f(i, j, (d + 2) % 4)] == best);
            ans += good;
        }
    }
